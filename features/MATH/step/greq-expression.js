...
When(/^I set the grade as Expression type and input "(.*)" equation$/, async function (eval) {
  ...
  await qa.input(page.math.raptorAms.gradeAs, 'Expression');
  ...
  await qa.executeScript(`const ta=document.querySelectorAll('textarea.ace_text-input'); ta[1].value='${evalGrade.EvalQ1.Expression}'; ta[1].dispatchEvent(new Event('input'))`);
  ...
  await qa.executeScript(`const ta=document.querySelectorAll('textarea.ace_text-input'); ta[0].value='${evalGrade.EvalQ1.Expression}'; ta[0].dispatchEvent(new Event('input'))`);
});
