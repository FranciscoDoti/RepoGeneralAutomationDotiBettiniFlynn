Feature: Writing happy path workflow student

Student complete the activities assigned
   
Background:
        Given I have opened Achieve "loginURL"

    @coureseware-logout
    Scenario: Verify that Student is able to attempt activities of a Instructor created pre-made assesment course created from Skills template 
        When I have logged in as "student"
        And I click on course card "E2E101"
        And I click on course plan in student account 
        And I click on pre-made assesment activity  
        And I answer the Questions in assement activity 
        And I validate the marks are displayed in gradebook 

    
    @coureseware-logout
    Scenario: Verify that Student is able to attempt activities of a Instructor custom made assesment course created from Skills Template
        When I have logged in as "student"
        And I click on course card "E2E101"
        And I click on course plan in student account
        And I click on custom made assement activity
        And I answer the Questions in assement activity 
        And I validate the marks are displayed in gradebook   
        And I validate the marks are displayed in gradebook 

    @coureseware-logout
    Scenario: Verify that Student is able to attempt activities of a Instructor created Reading and file course created from Skills Template
        When I have logged in as "student"
        And I click on course card "E2E101"
        And I click on course plan in student account
        And I click on Reading activity 
        And I validate the content is displayed
        And I click on file activity 
        And I validate that file is downloaded 
